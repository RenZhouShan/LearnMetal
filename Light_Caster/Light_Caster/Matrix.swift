...
struct Matrix4x4 {
    ...
    static func perspectiveProjection(_ aspect: Float32, fieldOfViewY: Float32,
        ...
        let zwScale = -1 * far * near / zRange
        
        mat.X.x = xScale
        mat.Y.y = yScale
        mat.Z.z = zScale//far / zRange
        mat.W.z = 1
        mat.Z.w = -1 * near * far / zRange
        mat.W.w = 0
    ...
    static func cameraView(_ cameraPos: SIMD4<Float>, _ targetPos: SIMD4<Float>,
        ...
        var mat = Matrix4x4()
        let ver = cameraPos - targetPos
        let zaxis = nor(SIMD3<Float>(ver.x, ver.y, ver.z))
        let xaxis = nor(cross(SIMD3<Float>(zaxis.x, zaxis.y, zaxis.z), SIMD3<Float>(lookUp.x, lookUp.y, lookUp.z)))
        let yaxis = nor(cross(SIMD3<Float>(xaxis.x, xaxis.y, xaxis.z), SIMD3<Float>(zaxis.x, zaxis.y, zaxis.z)))
        ...
        mat.W = simd_float4(-dot(targetPos, xaxis),
                            -dot(targetPos, yaxis),
                            -dot(targetPos, zaxis), 1)// */
...
struct Light{
    ...
    let flashLightPos:SIMD4<Float>;
    let flashLightDir:SIMD4<Float>;
    let flashLightCutOff:SIMD4<Float>;
}
