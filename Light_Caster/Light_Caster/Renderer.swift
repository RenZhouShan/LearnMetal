...
class Renderer: NSObject, MTKViewDelegate {
    ...
    init?(metalKitView: MTKView){
        ...
        CreateBuffers()
        SetTexture()
        CreateShaders(metalKitView: metalKitView)
        UpdateLight()
    }
    ...
    func CreateShaders(metalKitView: MTKView){
    ...
    func UpdateLight(){
        ...
        let flashLightPos = SIMD4<Float>(0, 0, 3, 1)
        let flashLightDir = SIMD4<Float>(0, 0, -1, 0)
        let flashLightCutOff = Float(30 / 180 * double_t.pi)
        let light = Light(lightPosition: lightPosition, color: lightColor, reflectivity: reflectivity, idensity: intensity, ambient: ambient, difuse: diffuse, specular: specular, shininess: SIMD4<Float>(repeating: shininess), flashLightPos: flashLightPos, flashLightDir: flashLightDir, flashLightCutOff: SIMD4<Float>(repeating: flashLightCutOff))
        let lightse = [light]
    ...
    func UpdateUniformBuffer(){
        ...
        let modelMatrices = ModelMatrices.init(SIMD3<Float>(0,0,-4.5), SIMD3<Float>(rotationAngle,rotationAngle,rotationAngle), SIMD3<Float>(1,1,1))
    ...
    func draw(in view: MTKView) {
        ...
        if let commandBuffer = commandQueue.makeCommandBuffer() {
            ...
            if let renderPassDescriptor = renderPassDescriptor, let commandEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: renderPassDescriptor) {
                ...
                renderPassDescriptor.depthAttachment.texture = device.makeTexture(descriptor: depthDesc)
                ...
                commandEncoder.drawPrimitives(type: .triangle, vertexStart: 6, vertexCount: 6, instanceCount: 1)
                commandEncoder.drawPrimitives(type: .triangle, vertexStart: 12, vertexCount: 6, instanceCount: 1)
                commandEncoder.drawPrimitives(type: .triangle, vertexStart: 18, vertexCount: 6, instanceCount: 1)
                commandEncoder.drawPrimitives(type: .triangle, vertexStart: 24, vertexCount: 6, instanceCount: 1)
                commandEncoder.drawPrimitives(type: .triangle, vertexStart: 30, vertexCount: 6, instanceCount: 1)*/
